    ...
    if (Predictions.length > 0) {
      ...
      for (var i = 0; i < Predictions.length; i++) {
        ...
        result.innerHTML += "[ " + i + " ] " + Predictions[i].class + ", " +
                            ...
                            Math.round(width) + ", " + Math.round(height) + "<br>";

        // Count selected object
        if (Predictions[i].class == object.value && Predictions[i].score >= score.value) {
          objectCount++;
        }

        // Trigger flash for specific objects
        if (["bottle", "tetra pack", "box", "keyboard","vase","rocket","paper","machine","car"].includes(Predictions[i].class)) {
          flashTriggered = true;
        }
      }
      count.innerHTML = objectCount;

      // Flash the light for 30 seconds if required
      if (flashTriggered) {
        fetch(location.origin + '/?flash=255;stop'); // turn on flash
        setTimeout(() => {
          fetch(location.origin + '/?flash=0;stop');  // turn off flash after 30 seconds
        }, 30000);
      }
